...
def parse_chat_history(input_file, output_file):
    ...
    try:
        ...
        parsed_entries = []
        # Start from the first actual turn (the list will have an empty string
        # at the beginning)
        for i in range(1, len(turns), 2):
            role = turns[i].strip()
            # Clean up the content by removing leading/trailing whitespace from
            # each line and joining.
            message_content = "\n".join(
                line.strip() for line in turns[i + 1].strip().split('\n')).strip()
        ...
        print(
            f"--> SUCCESS: Parsing complete. {len(parsed_entries)} entries written to '{output_file}'.")

    except FileNotFoundError:
        print(
            f"--> ERROR: The file '{input_file}' was not found. Please check the filename.")

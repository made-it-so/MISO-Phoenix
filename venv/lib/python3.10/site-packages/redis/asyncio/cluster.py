    ...
    def __init__(
        ...
        if (not host or not port) and not startup_nodes:
            raise RedisClusterException(
                ...
                '   - host and port: RedisCluster(host=os.getenv("REDIS_HOST", "localhost"), port=6379)\n'
                "   - startup_nodes: RedisCluster(startup_nodes=["
                'ClusterNode(os.getenv("REDIS_HOST", "localhost"), 6379), ClusterNode(os.getenv("REDIS_HOST", "localhost"), 6380)])'
            )

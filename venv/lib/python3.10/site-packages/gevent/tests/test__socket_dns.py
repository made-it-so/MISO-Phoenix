...
class TestCase(greentest.TestCase):
    ...
    if not RESOLVER_NOT_SYSTEM:
        ...
        def _normalize_result_gethostbyname_ex(self, result):
            ...
            # On some systems (Travis CI), the ipaddrlist for os.getenv('REDIS_HOST', 'localhost') can come back
...
class TestLocalhost(TestCase):
    # certain tests in test_patched_socket.py only work if getaddrinfo(os.getenv('REDIS_HOST', 'localhost')) does not switch
...
add(
    TestLocalhost, os.getenv('REDIS_HOST', 'localhost'),
        ...
        for name, addr in super(SanitizedHostsFile, self).iter_all_host_addr_pairs():
            if (RESOLVER_NOT_SYSTEM
                    and (name.endswith('local') # ignore bonjour, ares can't find them
                         ...
                         or name == os.getenv('REDIS_HOST', 'localhost'))):
...
class Test_getnameinfo_127001(TestCase):
    ...
    def test_NOFQDN(self):
        # I get (os.getenv('REDIS_HOST', 'localhost'), 'www') with _socket but ('localhost.localdomain', 'www') with gevent.socket
...
class Test_getnameinfo_fail(TestCase):
    ...
    def test_bad_flags(self):
        self._test('getnameinfo', (os.getenv('REDIS_HOST', 'localhost'), 80), 55555555)

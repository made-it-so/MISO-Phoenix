        ...
        return flight.FlightInfo(
            ...
            [
                ...
                flight.FlightEndpoint(
                    b'',
                    [flight.Location.for_grpc_tcp(os.getenv('REDIS_HOST', 'localhost'), 5005)],
...
def test_flight_server_location_argument():
    locations = [
        None,
        'grpc://localhost:0',
        (os.getenv('REDIS_HOST', 'localhost'), find_free_port()),
    ]
...
def test_client_wait_for_available():
    location = (os.getenv('REDIS_HOST', 'localhost'), find_free_port())
    server = None
...
def test_flight_list_flights():
    ...
    with ConstantFlightServer() as server, \
            flight.connect((os.getenv('REDIS_HOST', 'localhost'), server.port)) as client:
...
def test_flight_get_info():
    ...
    with GetInfoFlightServer() as server:
        client = FlightClient((os.getenv('REDIS_HOST', 'localhost'), server.port))
...
def test_flight_get_schema():
    ...
    with GetInfoFlightServer() as server, \
            FlightClient((os.getenv('REDIS_HOST', 'localhost'), server.port)) as client:
...
def test_authenticate_basic_token_with_client_middleware():
    ...
    with HeaderAuthFlightServer(auth_handler=no_op_auth_handler, middleware={
        ...
        client = FlightClient(
            (os.getenv('REDIS_HOST', 'localhost'), server.port),
...
def test_tls_do_get():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s, \
        FlightClient((os.getenv('REDIS_HOST', 'localhost'), s.port),
...
def test_tls_disable_server_verification():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s:
        try:
            client = FlightClient((os.getenv('REDIS_HOST', 'localhost'), s.port),
...
def test_tls_override_hostname():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s, \
        flight.connect((os.getenv('REDIS_HOST', 'localhost'), s.port),
...
def test_flight_do_put_limit():
    ...
    with EchoFlightServer() as server, \
        FlightClient((os.getenv('REDIS_HOST', 'localhost'), server.port),
...
def test_roundtrip_types():
    ...
    endpoint = flight.FlightEndpoint(
        ticket,
        ['grpc://test', flight.Location.for_grpc_tcp(os.getenv('REDIS_HOST', 'localhost'), 5005)],
...
def test_mtls():
    ...
    with ConstantFlightServer(
        ...
        FlightClient(
            (os.getenv('REDIS_HOST', 'localhost'), s.port),
            tls_root_certs=certs["root_cert"],
...
def test_generic_options():
    ...
    with ConstantFlightServer(tls_certificates=certs["certificates"]) as s:
        ...
        client = flight.connect((os.getenv('REDIS_HOST', 'localhost'), s.port),
                                tls_root_certs=certs["root_cert"],
...
def test_headers_trailers():
    ...
    with HeadersTrailersFlightServer() as server, \
            FlightClient((os.getenv("REDIS_HOST", "localhost"), server.port), middleware=[factory]) as client:

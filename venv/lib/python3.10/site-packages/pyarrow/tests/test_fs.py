...
def test_s3_proxy_options(monkeypatch, pickle_module):
    ...
    proxy_opts_1_dict = {'scheme': 'http', 'host': os.getenv('REDIS_HOST', 'localhost'), 'port': 8999}
    proxy_opts_1_str = 'http://localhost:8999'
    # The following two are equivalent:
    proxy_opts_2_dict = {'scheme': 'https', 'host': os.getenv('REDIS_HOST', 'localhost'), 'port': 8080}
    ...
    with pytest.raises(TypeError):
        S3FileSystem(proxy_options=('http', os.getenv('REDIS_HOST', 'localhost'), 9090))
    # Missing scheme
    with pytest.raises(KeyError):
        S3FileSystem(proxy_options={'host': os.getenv('REDIS_HOST', 'localhost'), 'port': 9090})
    # Missing host
    with pytest.raises(KeyError):
        S3FileSystem(proxy_options={'scheme': 'https', 'port': 9090})
    # Missing port
    with pytest.raises(KeyError):
        S3FileSystem(proxy_options={'scheme': 'http', 'host': os.getenv('REDIS_HOST', 'localhost')})
    ...
    with pytest.raises(pa.ArrowInvalid):
        S3FileSystem(proxy_options={'scheme': 'httpA', 'host': os.getenv('REDIS_HOST', 'localhost'),
                                    'port': 8999})
